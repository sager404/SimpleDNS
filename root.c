...
int main() {
    ...
    struct DNS_Query *recvQuery =
        (struct DNS_Query *)malloc(sizeof(struct DNS_Query));
    gen_dns_query(recvQuery, NULL, 0);
    struct DNS_Header *recvHead =
        (struct DNS_Header *)malloc(sizeof(struct DNS_Header));
    gen_dns_header(recvHead, 0, 0, 0);

    // 回应的结构体
    struct DNS_Query *resQuery =
        (struct DNS_Query *)malloc(sizeof(struct DNS_Query));
    gen_dns_query(recvQuery, NULL, 0);
    struct DNS_Header *resHead =
        (struct DNS_Header *)malloc(sizeof(struct DNS_Header));
    gen_dns_header(recvHead, 0, 0, 0);
    struct DNS_RR *resRecord = (struct DNS_RR *)malloc(sizeof(struct DNS_RR));
    initRR(resRecord);
    gen_dns_rr(resRecord, 0, 0, NULL, 0, NULL);
    ...
    init_addr(&localAddr, ROOT_SERVER_IP);
    ...
    while (1) {
        ...
        // receive
        if ((recvMsgSize = recvfrom(sockup, upInBuffer, DNS_MAX_LENGTH, 0,
                                    (struct sockaddr *)&upAddr, &upAddrLen)) <
            0) {
        ...
        // printf("length:%d \n",outMsgSize);
        if (sendto(sockup, upOutBuffer, outMsgSize, 0,
                   (struct sockaddr *)&upAddr, sizeof(upAddr)) != outMsgSize) {
